    ...
    Tokenizer.prototype = {
        ...
        parseFormInput: function () {
            ...
            for (var i = 0, j = 0; i < values.length; ++i) {
                if (values[i]) {
                    this.list.add(new Item(this.channel, values[i], j++), j++);
                }
            }
    ...
    Item.prototype = {
        ...
        initialize: function () {
            ...
            this.$element = $('<span class="label"></span>')
                .append(this.value)
                .append(this.$icon)
            .append("<input type='hidden' name='abcde[" + this.index + "].12345' value='" + this.value + "' />")
            .append("<input type='hidden' name='abcde[" + this.index + "]." + "IsNew" + "' value='" + this.new + "' />");
            //abcde = this.$formInput.attr("name").split(".")[0].split('[')[0]
            //12345 = this.$formInput.attr("name").split(".")[1]
